...
public class FPUMulTest {
	...
	public void fpuMulTest1(){
		src = new DataType("00111111100011001100110011001101" );
		dest = new DataType("00111111001100110011001100110011" );
		result = fpu.mul(src, dest);
		assertEquals( "00111111010001010001111010111000", result.toString() );
	}
	...
	public void fpuMulTest9() {
		...
		for (float v : input) {
			for (float value : input) {
				src = new DataType(Transformer.intToBinary(Integer.toString(Float.floatToIntBits(v))));
				dest = new DataType(Transformer.intToBinary(Integer.toString(Float.floatToIntBits(value))));
				result = fpu.mul(src, dest);
				String expect = Transformer.intToBinary(Integer.toString(Float.floatToIntBits(v * value)));
				if (!expect.equals(result.toString())) {
					System.out.println("i = " + v + ", j = " + value);
				...
				assertEquals(Transformer.intToBinary(Integer.toString(Float.floatToIntBits(v * value))), result.toString());
			}
		}
	}
