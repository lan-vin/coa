 ...
 */
public class ALU {
    public static String negHandle(String src){
    ...
    public DataType mul(DataType src, DataType dest) {
        ...
        DataType negDest=add(new DataType(negHandle(destValue)),extra1);//-X
        int cnt=0;
    ...
    public DataType div(DataType src, DataType dest) {
        ...
        if(!flag) {
            DataType extra1=new DataType(dest.fill32bit("1"));
            dest=add(new DataType(negHandle(dest.toString())),extra1);
        }
        remainderReg=R;
        return dest;
    }
