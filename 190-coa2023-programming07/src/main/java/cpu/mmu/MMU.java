...
public class MMU {
    ...
    private String toRealLinearAddr(String logicAddr) {
        // TODO
        String base= memory.fill32bit(logicAddr.substring(0,16)+"0000");
        DataType Base=new DataType(base);
        String offset= memory.fill32bit(logicAddr.substring(32));
        DataType Offset=new DataType(offset);
        return new ALU().add(Base,Offset).toString();
    }
    ...
    private String toSegLinearAddr(String logicAddr) {
        ...
        int Index= Integer.parseInt(Transformer.binaryToInt(index));
        char[] base=Memory.getMemory().getBaseOfSegDes(Index);
        String temp= memory.fill32bit(String.copyValueOf(base));
        String offset=logicAddr.substring(16);
        DataType Base=new DataType(temp);
        DataType Offset=new DataType(offset);
        return new ALU().add(Base,Offset).toString();
    }
    ...
    private String toPagePhysicalAddr(String linearAddr) {
        // TODO
        int vP= Integer.parseInt(Transformer.binaryToInt(linearAddr.substring(0,20)));
        String pP;
        if(TLB.isAvailable)pP=String.copyValueOf(tlb.getFrameOfPage(vP));
        else pP=String.copyValueOf(memory.getFrameOfPage(vP));
        String rest=linearAddr.substring(20);
        // TODO: add tlb here
