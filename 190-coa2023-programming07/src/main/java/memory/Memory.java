...
public class Memory {
    ...
    public String fill32bit(String s){//高位补0，凑32位
    ...
    public void real_load(String pAddr, int len) {
        // TODO
        if(Cache.isAvailable)Cache.getCache().write(pAddr,len,Disk.getDisk().read(pAddr,len));
        else getMemory().write(pAddr,len,Disk.getDisk().read(pAddr,len));
    }
    ...
    public void seg_load(int segIndex) {
        ...
        Arrays.fill(getSegDescriptor(segIndex).limit,'1');//填GDT对应段
        if(!PAGE){//未分页，直接加载数据
            char[] base=getBaseOfSegDes(segIndex);
            char[] limit=getLimitOfSegDes(segIndex);
            int len= Integer.parseInt(Transformer.binaryToInt(String.copyValueOf(limit)));
            getMemory().write(fill32bit("0"),len,Disk.getDisk().read(String.copyValueOf(base),len));
        }
    }
    ...
    public void page_load(int vPageNo) {
        // TODO
        getPageItem(vPageNo).isInMem=true;//虚拟页已经填入
        String vP=fill32bit(Transformer.intToBinary(String.valueOf(vPageNo))).substring(12);
        int i=0;
        while (pageValid[i]){i++;}
        String pP=fill32bit(Transformer.intToBinary(String.valueOf(i))).substring(12);//内存空闲的物理页框，20位
        pageValid[i]=true;
        getPageItem(vPageNo).pageFrame= pP.toCharArray();//虚拟页对应的物理页框初始化
        if(!Cache.isAvailable)getMemory().write(pP+"000000000000",PAGE_SIZE_B,Disk.getDisk().read(vP+"000000000000",PAGE_SIZE_B));
        else Cache.getCache().write(pP+"000000000000",PAGE_SIZE_B,Disk.getDisk().read(vP+"000000000000",PAGE_SIZE_B));
    }
